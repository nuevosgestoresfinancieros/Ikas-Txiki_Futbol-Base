        ...
        {filtered.length === 0 ? (
          ...
          <article key={`mobile-${p.id}`} className="surface-card p-4">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-2 text-sm">
              <div className="rounded-xl bg-slate-50 p-3"><dt className="text-xs font-bold uppercase text-slate-400">{t("equipmentShirtName")}</dt><dd className="mt-1 font-semibold text-slate-800">{p.nombre_camiseta || "—"}</dd></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[1320px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.length === 0 ? (
                <tr><td colSpan={17} className="px-4 py-10 text-center text-slate-400">{t("equipmentNoResults")}</td></tr>
              ) : filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className={`hover:bg-slate-50/80 transition-colors ${isEditing ? "bg-primary/3 ring-1 ring-inset ring-primary/20" : ""}`}>
                    ...
                    <td className="px-3 py-2.5">
                      <Cell label={t("equipmentShirtName")} value={row.nombre_camiseta} editing={isEditing} onChange={setField("nombre_camiseta")} />
                    </td>
                    <td className="px-3 py-2.5">
