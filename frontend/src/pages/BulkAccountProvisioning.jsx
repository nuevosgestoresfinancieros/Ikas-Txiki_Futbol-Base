...
export default function BulkAccountProvisioning({ type, onCancel, onCompleted }) {
  ...
  return <section className="space-y-4">
    ...
    {busy && !rows.length ? <p className="py-8 text-center text-slate-500">{t("loadingCandidates")}</p> : error ? <p role="alert" className="rounded-xl bg-red-50 p-3 text-red-800">{error}</p> : <div className="max-h-80 space-y-2 overflow-y-auto">{visible.map((row) => {
      const childNames = familyMode ? (row.children || []).map((child) => child.name).filter(Boolean) : [];
      const familyDetail = familyMode ? `${row.children?.length || 0} ${t("children").toLocaleLowerCase()}${childNames.length ? `: ${childNames.join(", ")}` : ""}` : row.family_name;
      return <label key={row[idKey]} className={`flex items-start gap-3 rounded-xl border p-3 ${eligible(row) ? "cursor-pointer" : "bg-slate-50 opacity-70"}`}><input type="checkbox" className="mt-1" disabled={!eligible(row)} checked={selected.includes(row[idKey])} onChange={() => toggle(row[idKey])} /><span className="min-w-0 flex-1"><strong className="block text-slate-900">{row[nameKey]}</strong><span className="block text-xs text-slate-500">{[row.category, row.email, familyDetail].filter(Boolean).join(" · ")}</span></span><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold">{statusLabels[row.status] || row.status}</span></label>;
    })}</div>}
