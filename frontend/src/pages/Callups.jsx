  ...
  return <div data-testid="callups-page">
    ...
    {loading ? <div className="surface-card p-8 text-center text-slate-500" role="status">{t("loading")}</div> : error ? <div className="surface-card p-8 text-center" role="alert"><p className="text-red-600">{error}</p><Button onClick={load} className="mt-4"><RotateCw className="h-4 w-4" />{t("retry")}</Button></div> : filtered.length === 0 ? <EmptyState icon={ClipboardList} message={t("noData")} /> : <div className="space-y-4">
      {filtered.map((callup) => <article key={callup.id} className="surface-card p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3"><div><h2 className="font-heading font-bold text-slate-900">{callup.equipo_nombre} vs {callup.match?.rival || "—"}</h2><p className="text-xs text-slate-500"><CalendarDays className="mr-1 inline h-3 w-3" />{callup.match?.fecha || "—"} · {callup.match?.hora || "--:--"} · {callup.match?.campo || callup.lugar_quedada || "—"}</p></div><div className="flex"><PermissionGate resource="callups" action="export"><Button variant="ghost" size="icon" onClick={() => downloadPdf(callup)} aria-label={t("downloadPdf")}><Download className="h-4 w-4" /></Button></PermissionGate><PermissionGate resource="callups" action="edit"><Button variant="ghost" size="icon" onClick={() => { setForm(callup); setDialog(true); }} aria-label={t("edit")}><Pencil className="h-4 w-4" /></Button></PermissionGate><PermissionGate resource="callups" action="delete"><Button variant="ghost" size="icon" onClick={() => remove(callup)} aria-label={t("delete")} className="text-red-600"><Trash2 className="h-4 w-4" /></Button></PermissionGate></div></div>
