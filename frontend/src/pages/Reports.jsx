    ...
    try {
      ...
      setProfessionalReport((current) => {
        ...
        if (next && next !== current) {
          const definition = reports.find((item) => item.id === next);
          setProfessionalFilters(initialReportFilters(next, definition?.filters));
        }
        return next || "";
      });
            ...
            <div className="min-w-0 space-y-4">
              <section className="surface-card min-w-0 overflow-hidden p-4" aria-labelledby="professional-filters-title">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {selectedDefinition?.filters.includes("type") && <label className="grid gap-1 text-sm font-semibold">{t("reportFilter_type")}<select value={professionalFilters.type || ""} onChange={(event) => changeProfessionalFilter({ type: event.target.value })} className="h-11 rounded-xl border px-3"><option value="">{t("all")}</option>{(reportOptions.types || []).map((value) => <option key={value} value={value}>{t(`reportValue_${value}`)}</option>)}</select></label>}
                  {selectedDefinition?.filters.includes("movement") && <label className="grid gap-1 text-sm font-semibold">{t("reportFilter_movement")}<select value={professionalFilters.movement || ""} onChange={(event) => changeProfessionalFilter({ movement: event.target.value })} className="h-11 rounded-xl border px-3"><option value="">{t("all")}</option>{(reportOptions.movements || []).map((value) => <option key={value} value={value}>{t(`reportValue_${value}`)}</option>)}</select></label>}
                  {selectedDefinition?.filters.includes("delivery") && <label className="grid gap-1 text-sm font-semibold">{t("reportFilter_delivery")}<select value={professionalFilters.delivery || ""} onChange={(event) => changeProfessionalFilter({ delivery: event.target.value })} className="h-11 rounded-xl border px-3"><option value="">{t("all")}</option>{(reportOptions.deliveries || []).map((value) => <option key={value} value={value}>{t(`reportValue_${value}`)}</option>)}</select></label>}
                  {selectedDefinition?.filters.includes("contact_type") && <label className="grid gap-1 text-sm font-semibold">{t("reportFilter_contact_type")}<select value={professionalFilters.contact_type || ""} onChange={(event) => changeProfessionalFilter({ contact_type: event.target.value })} className="h-11 rounded-xl border px-3"><option value="">{t("all")}</option>{(reportOptions.contact_types || []).filter((value) => value !== "all").map((value) => <option key={value} value={value}>{t(`reportValue_${value}`)}</option>)}</select></label>}
                  {selectedDefinition?.filters.includes("payment_method") && <label className="grid gap-1 text-sm font-semibold">{t("reportFilter_payment_method")}<select value={professionalFilters.payment_method || ""} onChange={(event) => changeProfessionalFilter({ payment_method: event.target.value })} className="h-11 rounded-xl border px-3"><option value="">{t("all")}</option>{(reportOptions.payment_methods || []).map((value) => <option key={value} value={value}>{t(`reportValue_${value}`)}</option>)}</select></label>}
