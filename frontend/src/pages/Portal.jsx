...
export default function Portal({ user }) {
  ...
  return <div data-testid="portal-page">
    ...
    <div className="mb-5 grid gap-4 lg:grid-cols-[1fr_1.4fr]">
      ...
      <Card title={t("nextActivity")} icon={Trophy} testid="portal-next-activity">{next ? <div className="rounded-2xl bg-gradient-to-r from-slate-900 to-[#173b58] p-5 text-white"><p className="text-xs font-bold uppercase tracking-widest text-teal-300">{t(`calendarType_${next.tipo}`)}</p><p className="mt-1 font-heading text-2xl font-bold">{next.titulo}</p><p className="mt-2 text-sm text-slate-200">{formatDate(next.fecha, { weekday: "long", day: "numeric", month: "long" })}{next.hora ? ` · ${next.hora}` : ""}{next.lugar ? ` · ${next.lugar}` : ""}</p></div> : <Empty>{t("noUpcomingActivities")}</Empty>}</Card>
    </div>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2">
      ...
      <Card title={t("attendance")} icon={ClipboardCheck} testid="portal-attendance"><div className="mb-4 rounded-2xl bg-teal-50 p-4"><p className="text-sm text-slate-600">{t("attendancePercentage")}</p><p className="font-heading text-3xl font-bold text-primary">{attendancePercentage(view.attendance)}%</p></div>{view.attendance.length ? <div className="space-y-2">{view.attendance.slice(0, 6).map((row) => <div key={`${row.training_id}-${row.player_id}`} className="flex items-center justify-between rounded-xl border border-slate-100 p-3"><span className="text-sm">{formatDate(row.date)}</span><StatusBadge status={row.status} /></div>)}</div> : <Empty>{t("noAttendanceRecords")}</Empty>}</Card>
