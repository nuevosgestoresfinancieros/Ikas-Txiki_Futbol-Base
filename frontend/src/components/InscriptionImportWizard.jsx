...
import { activeModalitiesFromApi, applyPreparationFilterChange, canApplyPreparationBulk, canFinalizeDraft, clearPreparationSelection, filterPreparationRecords, modalityOptionLabel, selectedOctoberIds, selectVisiblePreparationRecords } from "./importPreparationView";
...
const COPY = {
  es: {
    ...
    chooseModality: "Selecciona una modalidad", confirmModality: "¿Confirmas aplicar {modality} a {count} registros seleccionados?", modalityApplied: "Modalidad aplicada a {count} registros.", modalityError: "No se pudo aplicar la modalidad.",
    ...
    historicalMode: "Base histórica: simulación aislada", historicalHelp: "Las filas auxiliares se excluyen, los 54 casos sin equipo permanecen en revisión y la importación definitiva está bloqueada.",
  ...
  eu: {
    ...
    chooseModality: "Hautatu modalitate bat", confirmModality: "{modality} hautatutako {count} erregistrori aplikatzea baieztatzen duzu?", modalityApplied: "Modalitatea {count} erregistrori aplikatu zaie.", modalityError: "Ezin izan da modalitatea aplikatu.",
    ...
    historicalMode: "Datu-base historikoa: simulazio isolatua", historicalHelp: "Errenkada osagarriak baztertzen dira, talderik gabeko 54 kasuak berrikusteko geratzen dira eta behin betiko inportazioa blokeatuta dago.",
...
export default function InscriptionImportWizard({ open, onOpenChange, onImported }) {
  ...
  const [status, setStatus] = useState(""); const [modalities, setModalities] = useState([]);
  ...
  const loadLists = async () => {
    const [draftResponse, historyResponse, modalityResponse] = await Promise.all([api.get("/inscription-imports/staging"), api.get("/inscription-imports/history"), api.get("/modalities")]);
    setDrafts(draftResponse.data); setHistory(historyResponse.data); setModalities(activeModalitiesFromApi(modalityResponse.data));
  };
  ...
  const updateField = async (record, field) => {
    if (field === "modalidad") {
      setSelected([record.id]); setBulk({ field: "modalidad", value: modalities.some((item) => item.code === record.modalidad) ? record.modalidad : "" });
      return;
    }
  ...
  const applyBulk = async (field = bulk.field, value = bulk.value, ids = selected) => {
    ...
    const modality = modalities.find((item) => item.code === value);
    const confirmation = c.confirmModality.replace("{modality}", modalityOptionLabel(modality, lang)).replace("{count}", ids.length);
    const confirmSuggestion = field !== "modalidad" || (Boolean(modality) && window.confirm(confirmation));
    if (!confirmSuggestion) return;
    setError(""); setStatus("");
    try { refresh(await api.post(`/inscription-imports/staging/${draft.id}/bulk`, { record_ids: ids, field, value, confirm_suggestion: confirmSuggestion })); setSelected([]); setStatus(field === "modalidad" ? c.modalityApplied.replace("{count}", ids.length) : c.saveContinue); }
    catch (e) { setError(e.response?.data?.detail || (field === "modalidad" ? c.modalityError : "No se pudo aplicar.")); }
  };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[96vh] w-[calc(100vw-0.75rem)] max-w-7xl overflow-y-auto overflow-x-hidden" data-testid="import-preparation-dialog">
      ...
      {!draft ? <div className="rounded-2xl border-2 border-dashed p-8 text-center text-slate-500">{busy ? "…" : c.empty}</div> : <>
        {historical && <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950"><p className="font-bold">{c.historicalMode}</p><p className="mt-1">{c.historicalHelp}</p></div>}
        ...
        <div className="grid gap-2 rounded-2xl border p-3 sm:grid-cols-2 lg:grid-cols-6"><input aria-label={c.filter} placeholder={c.filter} value={filters.query} onChange={(e) => changeFilters({query: e.target.value})} className="h-11 rounded-xl border px-3" /><select aria-label={c.category} value={filters.category} onChange={(e) => changeFilters({category: e.target.value})} className="h-11 rounded-xl border px-3"><option value="">{c.allCategories}</option>{categories.map((value) => <option key={value}>{value}</option>)}</select><select aria-label={c.team} value={filters.team} onChange={(e) => changeFilters({team: e.target.value})} className="h-11 rounded-xl border px-3"><option value="">{c.allTeams}</option><option value="__missing__">{c.noTeam}</option>{teams.map((value) => <option key={value}>{value}</option>)}</select><input aria-label={c.age} type="number" min="4" max="30" placeholder={c.age} value={filters.age || ""} onChange={(e) => changeFilters({age: e.target.value})} className="h-11 rounded-xl border px-3" /><select aria-label={c.previousTeam} value={filters.previousTeam || ""} onChange={(e) => changeFilters({previousTeam: e.target.value})} className="h-11 rounded-xl border px-3"><option value="">{c.previousTeam}</option>{previousTeams.map((value) => <option key={value}>{value}</option>)}</select><select aria-label={c.allStates} value={filters.status || ""} onChange={(e) => changeFilters({status: e.target.value})} className="h-11 rounded-xl border px-3"><option value="">{c.allStates}</option><option value="incidents">{c.withIncidents}</option><option value="ready">{c.ready}</option></select></div>
        <section className="rounded-2xl border p-3"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><h3 className="font-heading font-bold" aria-live="polite">{c.bulk} · {selected.length} {c.selected}</h3><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => setSelected(selectVisiblePreparationRecords(visible))} disabled={!visible.length}>{c.selectVisible}</Button><Button type="button" variant="outline" onClick={() => setSelected(clearPreparationSelection())} disabled={!selected.length}>{c.clearSelection}</Button></div></div><div className="grid gap-2 sm:grid-cols-[180px_1fr_auto]"><select value={bulk.field} onChange={(e) => setBulk({field: e.target.value, value: ""})} className="h-11 rounded-xl border px-3"><option value="equipo">{c.team}</option><option value="categoria">{c.category}</option><option value="modalidad">{c.modality}</option></select>{bulk.field === "modalidad" ? <select aria-label={c.modality} value={bulk.value} onChange={(e) => setBulk({...bulk, value: e.target.value})} className="h-11 rounded-xl border px-3"><option value="">{c.chooseModality}</option>{modalities.map((item) => <option key={item.code} value={item.code}>{modalityOptionLabel(item, lang)}</option>)}</select> : <input value={bulk.value} onChange={(e) => setBulk({...bulk, value: e.target.value})} className="h-11 rounded-xl border px-3" />}<Button onClick={() => applyBulk()} disabled={!canApplyPreparationBulk(selected, bulk, modalities)}>{c.apply}</Button></div></section>
        ...
        {historical && <section className="grid gap-3 lg:grid-cols-2"><div><h3 className="mb-2 font-heading font-bold">{c.fuzzy}</h3>{draft.fuzzy_matches?.filter((item) => !item.decision).map((group) => <div key={group.id} className="mb-2 flex flex-wrap items-center justify-between gap-2 rounded-xl border p-3 text-sm"><span>{group.record_ids.length} · {Math.round((group.score || 0) * 100)}%</span><span className="flex gap-1"><Button size="sm" variant="outline" onClick={() => resolveHistorical("fuzzy", group, "same_person")}>{c.reviewSame}</Button><Button size="sm" variant="ghost" onClick={() => resolveHistorical("fuzzy", group, "different_people")}>{c.reviewDifferent}</Button></span></div>)}</div><div><h3 className="mb-2 font-heading font-bold">{c.families}</h3>{draft.family_candidates?.filter((item) => !item.decision).slice(0, 20).map((group) => <div key={group.id} className="mb-2 flex flex-wrap items-center justify-between gap-2 rounded-xl border p-3 text-sm"><span>{group.record_ids.length} · {group.signals?.length || 0}</span><span className="flex gap-1"><Button size="sm" variant="outline" onClick={() => resolveHistorical("family", group, "confirm_shared")}>{c.reviewShared}</Button><Button size="sm" variant="ghost" onClick={() => resolveHistorical("family", group, "keep_separate")}>{c.reviewSeparate}</Button></span></div>)}</div></section>}
