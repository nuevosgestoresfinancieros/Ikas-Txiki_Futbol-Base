...
import { activeModalitiesFromApi, activeTeamsFromApi, applyPreparationFilterChange, canApplyPreparationBulk, canFinalizeDraft, clearPreparationSelection, existingCategoriesFromApi, filterPreparationRecords, importPreparationError, isUnresolvedFuzzyMatch, MODALITY_CONTROL_COPY, modalityAssignmentDisabledReason, modalityOptionLabel, officialModalityCode, selectedOctoberIds, selectVisiblePreparationRecords, teamsForCategory } from "./importPreparationView";
...
const IMPORT_PREPARATION_COPY = {
  es: {
    ...
    suggestion: "Sugerencia provisional según la categoría; requiere confirmación administrativa", acceptSuggestion: "Aceptar sugerencia para la selección", acceptSelectedSuggestions: "Aceptar sugerencias seleccionadas ({count})", confirmSelectedSuggestions: "¿Confirmas la modalidad sugerida para {count} registros seleccionados?",
  ...
  eu: {
    ...
    suggestion: "Kategoriaren araberako behin-behineko iradokizuna; administratzaileak baieztatu behar du", acceptSuggestion: "Onartu iradokizuna hautatutakoentzat", acceptSelectedSuggestions: "Onartu hautatutako iradokizunak ({count})", confirmSelectedSuggestions: "Baieztatu hautatutako {count} erregistroen iradokitako modalitatea?",
...
export default function InscriptionImportWizard({ open, onOpenChange, onImported }) {
  ...
  const selectedSuggestedRecords = useMemo(() => records.filter((record) => selected.includes(record.id) && !record.modalidad && modalities.some((item) => item.code === record.modality_suggestion)), [records, selected, modalities]);
  ...
  const acceptSelectedSuggestions = async () => {
    if (!selectedSuggestedRecords.length) return;
    const count = selectedSuggestedRecords.length;
    if (!window.confirm(c.confirmSelectedSuggestions.replace("{count}", count))) return;
    const groups = selectedSuggestedRecords.reduce((result, record) => {
      (result[record.modality_suggestion] ||= []).push(record.id); return result;
    }, {});
    setBusy(true); setError(""); setStatus("");
    try {
      let response;
      for (const [value, recordIds] of Object.entries(groups)) {
        response = await api.post(`/inscription-imports/staging/${draft.id}/bulk`, { record_ids: recordIds, field: "modalidad", value, confirm_suggestion: true });
      }
      refresh(response); setSelected([]); setStatus(c.assignmentApplied.replace("{count}", count));
    } catch (e) {
      try { refresh(await api.get(`/inscription-imports/staging/${draft.id}`)); } catch (_) { /* conserva el error original */ }
      setError(e.response?.data?.detail || c.assignmentError);
  ...
  const finalize = async () => {
    ...
    try { const result = await api.post(`/inscription-imports/staging/${draft.id}/confirm`, { confirmed: true }); const summary = result?.data?.summary; toast.success(summary ? `${summary.matched_players} fichas · ${summary.team_assignments || 0} equipos asignados · ${summary.bank_references} cuentas protegidas · ${summary.unmatched_rows + summary.ambiguous_rows} pendientes` : c.final); setDraft(null); await loadLists(); onImported?.(); }
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[96vh] w-[calc(100vw-0.75rem)] max-w-7xl overflow-y-auto overflow-x-hidden" data-testid="import-preparation-dialog">
      ...
      {!draft ? <div className="rounded-2xl border-2 border-dashed p-8 text-center text-slate-500">{busy ? "…" : c.empty}</div> : <>
        ...
        {historical && <div className="-mt-1 flex justify-end"><Button type="button" onClick={acceptSelectedSuggestions} disabled={!selectedSuggestedRecords.length || busy}>{c.acceptSelectedSuggestions.replace("{count}", selectedSuggestedRecords.length)}</Button></div>}
