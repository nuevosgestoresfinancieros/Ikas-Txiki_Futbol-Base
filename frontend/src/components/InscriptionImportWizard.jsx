  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[96vh] w-[calc(100vw-0.75rem)] max-w-7xl overflow-y-auto overflow-x-hidden" data-testid="import-preparation-dialog">
      ...
      {!draft ? <div className="rounded-2xl border-2 border-dashed p-8 text-center text-slate-500">{busy ? "…" : c.empty}</div> : <>
        ...
        {historical && <section className="grid gap-3 lg:grid-cols-2"><div><h3 className="mb-2 font-heading font-bold">{c.fuzzy}</h3>{draft.fuzzy_matches?.filter((item) => !item.decision || item.decision === "leave_pending").map((group) => <div key={group.id} className="mb-2 flex flex-wrap items-center justify-between gap-2 rounded-xl border p-3 text-sm"><span>{group.record_ids.length} · {Math.round((group.score || 0) * 100)}%</span><span className="flex gap-1"><Button size="sm" variant="outline" onClick={() => resolveHistorical("fuzzy", group, "same_person")}>{c.reviewSame}</Button><Button size="sm" variant="ghost" onClick={() => resolveHistorical("fuzzy", group, "different_people")}>{c.reviewDifferent}</Button><Button size="sm" variant="ghost" onClick={() => resolveHistorical("fuzzy", group, "leave_pending")}>{c.leavePending}</Button></span></div>)}</div><div><h3 className="mb-2 font-heading font-bold">{c.families}</h3>{draft.family_candidates?.filter((item) => !item.decision).slice(0, 20).map((group) => <div key={group.id} className="mb-2 flex flex-wrap items-center justify-between gap-2 rounded-xl border p-3 text-sm"><span>{group.record_ids.length} · {group.signals?.length || 0}</span><span className="flex gap-1"><Button size="sm" variant="outline" onClick={() => resolveHistorical("family", group, "confirm_shared")}>{c.reviewShared}</Button><Button size="sm" variant="ghost" onClick={() => resolveHistorical("family", group, "keep_separate")}>{c.reviewSeparate}</Button></span></div>)}</div></section>}
