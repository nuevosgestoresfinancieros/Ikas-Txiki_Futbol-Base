...
export default function NotificationCenter({ user, dark = false }) {
  ...
  const [preferences, setPreferences] = useState(() => ({ in_app: true, email: true, telegram: true, callups: true, schedule_changes: true, payments: true, documents: true, ...(user?.notification_preferences || {}) }));
  ...
  return <>
    <Button ref={triggerRef} variant="ghost" size="icon" aria-label={t("notifications")} onClick={() => setOpen(true)} className={`relative ${dark ? "text-white hover:bg-white/10 hover:text-white" : "bg-white text-slate-700 shadow-md hover:bg-slate-50"}`}>
    ...
    <Sheet open={open} onOpenChange={setOpen}><SheetContent
      side="right"
      closeLabel={t("close")}
      ...
      className="w-[min(94vw,26rem)] max-w-none overflow-y-auto p-0"
    >
